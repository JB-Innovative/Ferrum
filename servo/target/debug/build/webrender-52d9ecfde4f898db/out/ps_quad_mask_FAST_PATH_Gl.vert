...
void main ()
{
  ...
  ivec2 tmpvar_58;
  tmpvar_58.x = int((uint(aClipData.y) % 1024u));
  tmpvar_58.y = int((uint(aClipData.y) / 1024u));
  vec4 tmpvar_59;
  tmpvar_59 = texelFetchOffset (sGpuBufferF, tmpvar_58, 0, ivec2(0, 0));
  mat4 transform_m_60;
  int tmpvar_61;
  tmpvar_61 = (aClipData.x & 8388607);
  ivec2 tmpvar_62;
  tmpvar_62.x = int((8u * (
    uint(tmpvar_61)
   % 128u)));
  tmpvar_62.y = int((uint(tmpvar_61) / 128u));
  transform_m_60[0] = texelFetchOffset (sTransformPalette, tmpvar_62, 0, ivec2(0, 0));
  transform_m_60[1] = texelFetchOffset (sTransformPalette, tmpvar_62, 0, ivec2(1, 0));
  transform_m_60[2] = texelFetchOffset (sTransformPalette, tmpvar_62, 0, ivec2(2, 0));
  transform_m_60[3] = texelFetchOffset (sTransformPalette, tmpvar_62, 0, ivec2(3, 0));
  vec4 tmpvar_63;
  tmpvar_63.zw = vec2(0.0, 1.0);
  tmpvar_63.xy = tmpvar_52;
  vClipLocalPos = (transform_m_60 * tmpvar_63);
  vClipMode.x = texelFetchOffset (sGpuBufferF, tmpvar_58, 0, ivec2(2, 0)).x;
  vec2 tmpvar_64;
  tmpvar_64 = (0.5 * (tmpvar_59.zw - tmpvar_59.xy));
  vClipLocalPos.xy = (vClipLocalPos.xy - ((tmpvar_64 + tmpvar_59.xy) * vClipLocalPos.w));
  v_clip_size = tmpvar_64;
  v_clip_radii = texelFetchOffset (sGpuBufferF, tmpvar_58, 0, ivec2(1, 0));
}
