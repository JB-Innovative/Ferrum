...
void main ()
{
  ...
  vec2 tmpvar_39;
  tmpvar_39 = ((tmpvar_35 - tmpvar_5.xy) / (tmpvar_5.zw - tmpvar_5.xy));
  float tmpvar_40;
  if (((tmpvar_26 & 1) != 0)) {
    tmpvar_40 = 1.0;
  } else {
  ...
  v_perspective.x = tmpvar_40;
  v_op.x = tmpvar_11.x;
  vec2 inv_texture_size_41;
  ivec2 tmpvar_42;
  ...
  inv_texture_size_41 = (1.0/(vec2(textureSize (sColor0, 0))));
  int tmpvar_44;
  tmpvar_44 = (tmpvar_11.y + 2);
  ivec2 tmpvar_45;
  tmpvar_45.x = int((uint(tmpvar_44) % 1024u));
  tmpvar_45.y = int((uint(tmpvar_44) / 1024u));
  vec4 tmpvar_46;
  tmpvar_46 = mix (mix (texelFetchOffset (sGpuCache, tmpvar_45, 0, ivec2(0, 0)), texelFetchOffset (sGpuCache, tmpvar_45, 0, ivec2(1, 0)), tmpvar_39.x), mix (texelFetchOffset (sGpuCache, tmpvar_45, 0, ivec2(2, 0)), texelFetchOffset (sGpuCache, tmpvar_45, 0, ivec2(3, 0)), tmpvar_39.x), tmpvar_39.y);
  vec4 tmpvar_47;
  tmpvar_47.xy = (tmpvar_43.xy + vec2(0.5, 0.5));
  tmpvar_47.zw = (tmpvar_43.zw - vec2(0.5, 0.5));
  v_backdrop_uv = (mix (tmpvar_43.xy, tmpvar_43.zw, (tmpvar_46.xy / tmpvar_46.w)) * inv_texture_size_41);
  v_backdrop_uv_sample_bounds = (tmpvar_47 * inv_texture_size_41.xyxy);
  vec2 inv_texture_size_48;
  ivec2 tmpvar_49;
  tmpvar_49.x = int((uint(tmpvar_11.z) % 1024u));
  tmpvar_49.y = int((uint(tmpvar_11.z) / 1024u));
  vec4 tmpvar_50;
  tmpvar_50 = texelFetchOffset (sGpuCache, tmpvar_49, 0, ivec2(0, 0));
  inv_texture_size_48 = (1.0/(vec2(textureSize (sColor1, 0))));
  int tmpvar_51;
  tmpvar_51 = (tmpvar_11.z + 2);
  ivec2 tmpvar_52;
  tmpvar_52.x = int((uint(tmpvar_51) % 1024u));
  tmpvar_52.y = int((uint(tmpvar_51) / 1024u));
  vec4 tmpvar_53;
  tmpvar_53 = mix (mix (texelFetchOffset (sGpuCache, tmpvar_52, 0, ivec2(0, 0)), texelFetchOffset (sGpuCache, tmpvar_52, 0, ivec2(1, 0)), tmpvar_39.x), mix (texelFetchOffset (sGpuCache, tmpvar_52, 0, ivec2(2, 0)), texelFetchOffset (sGpuCache, tmpvar_52, 0, ivec2(3, 0)), tmpvar_39.x), tmpvar_39.y);
  vec4 tmpvar_54;
  tmpvar_54.xy = (tmpvar_50.xy + vec2(0.5, 0.5));
  tmpvar_54.zw = (tmpvar_50.zw - vec2(0.5, 0.5));
  v_src_uv = ((mix (tmpvar_50.xy, tmpvar_50.zw, 
    (tmpvar_53.xy / tmpvar_53.w)
  ) * inv_texture_size_48) * mix (tmpvar_37.w, 1.0, tmpvar_40));
  v_src_uv_sample_bounds = (tmpvar_54 * inv_texture_size_48.xyxy);
}
